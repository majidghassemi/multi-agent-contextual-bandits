...
second in the only way available at simulable horizons, by showing
that the anytime bonus remains far above $\gamma/3$ at every $N$
tested.
\end{remark}
...
\paragraph{Exp.~5 (behavioural collapse is not reachable at simulable
horizons, as predicted).}
...
$\geq\Delta$.  Sweeping $N\in\{2,\dots,32\}$ at $c_\beta=0.15$ with
$(\gamma,\Delta)=(0.01,0.01)$, the event is \emph{censored} at every
$N$ tested. no sustained entry occurs before the horizon.  The reason
is quantitative and is exactly Remark~\ref{rem:noN}. The per-agent UCB
bonus at $T=1.2\times10^4$ is $\approx0.87$ against
$\gamma/3=3.3\times10^{-3}$, more than two orders of magnitude too
large, and it carries no $1/N$ speedup, so no accessible $N$ helps.
The estimation-side quantities do collapse on schedule
(Exp.~1), and the epoch variant, which has no bonus, collapses
behaviourally within reach (Exp.~R1).  We therefore report no fitted
$N$-exponent for the anytime behavioural collapse time. A censored
measurement is not a measurement, and the honest statement is that
Theorem~\ref{thm:cec}'s behavioural conclusion is verified for the
epoch variant and remains asymptotic for the anytime one.
...
featurized gate.  Exp.~5 is a negative instrumentation result and is
reported as such.
...
$m_k=Nt_1 2^{k-1}$ only.  Together with Exp.~5, which shows the
anytime bonus stuck far above $\gamma/3$ at every $N$, this is the
predicted separation between the two algorithms.
...
claim for them. Exp.~5 confirms the practical consequence. The
behavioural collapse of the anytime algorithm is not reachable in
simulation.
