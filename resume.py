#!/usr/bin/env python3
"""
Resume experiments from exp3 onward.
Loads cached exp1/exp2 data, runs exp3-7, generates all figures.
"""
...
# ============================================================================
# FRAMEWORK (duplicated from main.py so we don't import it)
# ============================================================================
d_ctx, K_acts = 4, 3
d_phi = d_ctx * K_acts
...
def complete(N):
    return gmat(nx.complete_graph(N)) if N > 1 else np.eye(1)


def cycle(N):
    return gmat(nx.cycle_graph(N))


def path(N):
    return gmat(nx.path_graph(N))
...
class DCESA:
    ...
    def update(self, ctx, ac, rw):
        ...
        for i in range(self.N):
            for j in range(self.N):
                if self.W[i, j] > 0 or i == j:
                    Wt[i, j] = self.trust[i, j] * self.W[i, j]
            rs = Wt[i].sum()
            Wt[i] /= rs if rs > 0 else 1.0
        Ai2 = self.Ai.copy()
        for i in range(self.N):
            p = phi_feat(ctx[i], ac[i])
            Ap = Ai2[i] @ p
...
# ============================================================================
# LOAD CACHED EXP1 & EXP2
# ============================================================================
print("Loading cached exp1 and exp2...")
with open(f'{OUTDIR}/exp1_data.pkl', 'rb') as f:
    exp1 = pickle.load(f)
with open(f'{OUTDIR}/exp2_data.pkl', 'rb') as f:
    exp2 = pickle.load(f)
print("  Loaded OK.")

# ============================================================================
# EXPERIMENT 3: Phase Transition in gamma(W)  [FIXED]
# ============================================================================
print("\n" + "=" * 60)
print("EXPERIMENT 3: Phase Transition in gamma(W)")
print("=" * 60)
...
for gn, W in graphs.items():
    ...
    for s in range(N_SEEDS):
        _, cr = run_naive(N, W, T_EXP34, s * 300 + N)
        regs.append(cr[-1])
    exp3[gn] = {'g': g, 'r': np.array(regs), 'm': np.mean(regs), 's': np.std(regs)}
    print(f"  {gn:12s}: gamma = {g:.4f}, R_T = {exp3[gn]['m']:.1f} +- {exp3[gn]['s']:.1f}")
...
# ============================================================================
# EXPERIMENT 5: Axiom Propagation
# ============================================================================
print("\n" + "=" * 60)
print("EXPERIMENT 5: Axiom Propagation")
print("=" * 60)
...
for gn, W in graphs.items():
    ...
    print(f"  {gn:12s}: gamma = {g:.4f}, ratio = {exp7[gn]['ratio']:.2f}x")
...
axes[0].set_ylabel('||theta_t - theta*||')
axes[0].set_title('Distance to theta*')
axes[0].legend()
...
Ns = sorted(exp2.keys())
ax.bar(range(len(Ns)), [exp2[N]['m'] for N in Ns], tick_label=[str(N) for N in Ns],
       color='steelblue', alpha=0.7)
ax.set_xlabel('N (agents)')
ax.set_ylabel('Collapse Time t*')
ax.set_title('Collapse Time ~ 1/N')
...
ax.set_xscale('log')
ax.set_xlabel('Spectral Gap gamma(W)')
ax.set_ylabel('Network Regret R_T')
ax.set_title('Phase Transition')
ax.grid(True, alpha=0.3)
for gn in gn_order:
    ax.annotate(gn, (exp3[gn]['g'], exp3[gn]['m']), textcoords="offset points",
...
for cfg in ['super', 'sub']:
    cr = exp4[cfg]['cr']
    mean_cr = cr.mean(0)
    ax.loglog(range(1, len(mean_cr) + 1), mean_cr / 16,
              label=f"{cfg} ({exp4[cfg]['gn']})", linewidth=2)
ax.loglog(range(1, T_EXP34 + 1), 0.1 * np.arange(1, T_EXP34 + 1) ** 1.0,
          'k--', alpha=0.3, label='O(T)')
ax.loglog(range(1, T_EXP34 + 1), 0.5 * np.arange(1, T_EXP34 + 1) ** 0.5,
          'k:', alpha=0.3, label='O(sqrt(T))')
ax.set_xlabel('Round t')
ax.set_ylabel('Normalized Regret')
ax.set_title('Regret Scaling')
ax.legend()
...
ax.set_yticks(range(len(Kb_sorted)))
ax.set_yticklabels([f'K={Kb}' for Kb in Kb_sorted])
ax.set_xlabel('Agent Index')
ax.set_title(f'Axiom Propagation (target p_bar = {exp5[Kb_sorted[-1]]["pbar"]})')
plt.colorbar(im, ax=ax, label='Effective Axiom Rate')
save(fig, 'fig_exp5_axiom_propagation.png')

# --- Figure 6: Exp 6 ---
fig, ax = plt.subplots(figsize=(7, 5))
pvals = sorted(exp6.keys())
ax.errorbar(pvals, [exp6[p]['m'] for p in pvals], yerr=[exp6[p]['s'] for p in pvals],
            marker='o', capsize=5, linewidth=2, markersize=8)
ax.set_xscale('log')
ax.set_xlabel('Axiom Rate p_bar')
ax.set_ylabel('Regret R_T')
ax.set_title('D-CESA Regret vs p_bar')
ax.grid(True, alpha=0.3)
save(fig, 'fig_exp6_dcesa_pbar.png')

# --- Figure 7: Exp 7 ---
fig, ax = plt.subplots(figsize=(7, 5))
gns = list(exp7.keys())
ax.plot([exp7[gn]['g'] for gn in gns], [exp7[gn]['ratio'] for gn in gns],
        marker='o', linewidth=2, markersize=10, color='green')
ax.set_xscale('log')
ax.set_xlabel('Spectral Gap gamma(W)')
ax.set_ylabel('Naive / D-CESA Ratio')
ax.set_title('Spectral-Gap Inversion')
ax.grid(True, alpha=0.3)
for gn in gns:
    ax.annotate(gn, (exp7[gn]['g'], exp7[gn]['ratio']), textcoords="offset points",
...
for N in [1, 4, 8, 16]:
    ax1.plot(exp1[N]['te'].mean(0), label=f'N={N}', alpha=0.8)
ax1.set_xlabel('Round t')
ax1.set_ylabel('||theta_t - theta*||')
ax1.set_title('(a) Biased Fixed Point')
ax1.legend(fontsize=8)
ax1.grid(True, alpha=0.3)
# (b) Exp 1 - regret
ax2 = plt.subplot(2, 4, 2)
for N in [1, 4, 8, 16]:
    ax2.plot(exp1[N]['cr'].mean(0), label=f'N={N}', alpha=0.8)
ax2.set_xlabel('Round t')
ax2.set_ylabel('Cumulative Regret')
ax2.set_title('(b) Regret vs N')
ax2.legend(fontsize=8)
ax2.grid(True, alpha=0.3)
# (c) Exp 2
gs = sorted(exp2.keys())
ax3 = plt.subplot(2, 4, 3)
ax3.bar(range(len(gs)), [exp2[N]['m'] for N in gs], tick_label=[str(N) for N in gs],
        color='steelblue', alpha=0.7)
ax3.set_xlabel('N')
ax3.set_ylabel('t*')
ax3.set_title('(c) Collapse Time')
ax3.grid(True, alpha=0.3, axis='y')
# (d) Exp 3
gno = sorted(exp3.keys(), key=lambda k: exp3[k]['g'], reverse=True)
ax4 = plt.subplot(2, 4, 4)
ax4.errorbar([exp3[k]['g'] for k in gno], [exp3[k]['m'] for k in gno],
             yerr=[exp3[k]['s'] for k in gno], marker='o', capsize=4, linewidth=2, markersize=6)
ax4.set_xscale('log')
ax4.set_xlabel('gamma(W)')
ax4.set_ylabel('R_T')
...
for cfg in ['super', 'sub']:
    ax5.loglog(range(1, T_EXP34 + 1), exp4[cfg]['cr'].mean(0) / 16, label=cfg, linewidth=2)
ax5.loglog(range(1, T_EXP34 + 1), 0.1 * np.arange(1, T_EXP34 + 1), 'k--', alpha=0.3, label='O(T)')
ax5.set_xlabel('t')
ax5.set_ylabel('R_t/N')
ax5.set_title('(e) Regret Scaling')
ax5.legend(fontsize=8)
ax5.grid(True, alpha=0.3, which='both')
# (f) Exp 5
ax6 = plt.subplot(2, 4, 6)
# FIXED
ax6.bar(range(len(exp5[200]['eff'])), exp5[200]['eff'], color='coral', alpha=0.7)
ax6.axhline(y=exp5[200]['pbar'] / 20, color='red', linestyle='--', label='target')
ax6.set_xlabel('Agent')
ax6.set_ylabel('Effective p')
ax6.set_title('(f) Axiom Equalization')
ax6.legend(fontsize=8)
ax6.grid(True, alpha=0.3)
# (g) Exp 6
ax7 = plt.subplot(2, 4, 7)
pv = sorted(exp6.keys())
ax7.errorbar(pv, [exp6[p]['m'] for p in pv], yerr=[exp6[p]['s'] for p in pv],
             marker='o', capsize=4, linewidth=2, markersize=6)
ax7.set_xscale('log')
ax7.set_xlabel('p_bar')
ax7.set_ylabel('R_T')
ax7.set_title('(g) D-CESA vs p_bar')
ax7.grid(True, alpha=0.3)
# (h) Exp 7
gns = list(exp7.keys())
ax8 = plt.subplot(2, 4, 8)
ax8.plot([exp7[gn]['g'] for gn in gns], [exp7[gn]['ratio'] for gn in gns],
         marker='o', linewidth=2, markersize=8, color='green')
ax8.set_xscale('log')
ax8.set_xlabel('gamma(W)')
ax8.set_ylabel('Ratio')
ax8.set_title('(h) Gap Inversion')
